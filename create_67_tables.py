#!/usr/bin/env python3
"""
Create All 67 Tables for Murray Restaurant
This script manually creates all 67 tables with proper positioning
...
class TableCreator:
    """Creates all 67 tables for Murray restaurant"""
    ...
    def create_all_67_tables(self) -> List[TableInfo]:
        """Create all 67 tables based on the floor plan layout"""
        print("Creating all 67 tables for Murray Restaurant...")
...
INSERT INTO tables (
    ...
    def save_results(self, output_dir: str = "detection_results"):
        ...
        with open(f"{output_dir}/all_67_tables.json", "w") as f:
            json.dump(tables_data, f, indent=2, default=str)
        
        # Save SQL script
        sql_script = self.generate_database_script()
        with open(f"{output_dir}/insert_all_67_tables.sql", "w") as f:
            f.write(sql_script)
    ...
    def print_summary(self):
        ...
        print("\n" + "="*60)
        print("ALL 67 TABLES CREATED SUCCESSFULLY")
        print("="*60)
...
def main():
    """Main function to create all 67 tables"""
    print("Creating All 67 Tables for Murray Restaurant")
    print("=" * 50)
    ...
    # Create all tables
    tables = creator.create_all_67_tables()
    ...
    if tables:
        ...
        print("\n✅ All 67 tables created successfully!")
        print("📁 Results saved to 'detection_results/' directory")
        print("🗄️  Use 'insert_all_67_tables.sql' to populate your database")
        print("🎯 Ready to make the floor map interactive!")
    else:
        print("❌ Failed to create tables. Please check the creation logic.")
